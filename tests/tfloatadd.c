...
int main(void) {
	...
	MCALIB_T = 0;
	floatval_t a = 1.0, b = 1.0, eps_a, eps_b, eps_r, check, test;
	srand(0L);
	eps_a = pow(2, floor(log2(a)) + 2) * get_rand();
	eps_b = pow(2, floor(log2(b)) + 2) * get_rand();
	check = (a + eps_a) + (b + eps_b);
	eps_r = pow(2, floor(log2(abs(check))) + 2) * get_rand();
	check += eps_r;
	srand(0L);
	test = _floatadd(1.0, 1.0);
	if (test != check) {
		exit(1);
	}
	return 0;
